...
def draw():
    ...
    if gameStatus == 1: # jogando
        ...
        screen.draw.text("FASE " + str(level) , midtop=(400, 10), owidth=0.5, ocolor=(255,255,255), color=(0,64,255) , fontsize=60)
        drawLives()
        if player.status >= 30:
            if player.lives > 0:
                drawCentreText("ATINGIDO!\nPressione Enter to continuar")
            else:
                drawCentreText("FIM DE JOGO!\nPressione Enter to continuar")
        if len(aliens) == 0 :
            drawCentreText("FASE COMPLETADA!\nPressione Enter para a fase seguinte")
    if gameStatus == 2: # fim de jogo, exibe placar
        drawHighScore()

def drawCentreText(t): # centraliza o texto
...
def drawHighScore():
    global highScore
    y = 0
    screen.draw.text("PLACAR GERAL", midtop=(400, 30), owidth=0.5, ocolor=(255,255,255), color=(0,64,255) , fontsize=60)
    for line in highScore:
        if y < 400:
            screen.draw.text(line, midtop=(400, 100+y), owidth=0.5, ocolor=(0,0,255), color=(255,255,0) , fontsize=50)
            y += 50
    screen.draw.text("Pressione Esc para jogar novamente" , center=(400, 550), owidth=0.5, ocolor=(255,255,255), color=(255,64,0) , fontsize=60)
